...
static void ram_tasklet_func(unsigned long priv){
	...
	ram->status = readl(ram->membase +STATUS_REG);
	...
	switch(ram->status){
		case 0x00:	
			...
			// rst read ptr
			//readl(ram->membase +RESET_RAM_REG);
...
ssize_t ram_read(struct file *filep, char __user *buffer, size_t len, loff_t *offset) {
	...
	if(sdev->started == false){
		sdev->started = true;
		writel(START, sdev->membase +START_REG);
	}
...
static long ram_ioctl(struct file *filep, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd){
		case START_CMD:
			...
			break;
		case STOP_CMD:
			writel(STOP, sdev->membase + START_REG);
			break;
		case STATUS_CMD:
			status = readl(sdev->membase +STATUS_REG);
			put_user(status, (int __user *)arg);
			break;
		case RESET_RAM_CMD:
			//readl(sdev->membase+ RESET_RAM_REG);
...
int ram_release(struct inode *inode, struct file *filep) {
	...
	writel(STOP, ram->membase +START_REG);
...
MODULE_ALIAS("iqram");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("RAM CTRL DRIVER");
