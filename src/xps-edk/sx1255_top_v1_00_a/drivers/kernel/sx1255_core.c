...
static long sx1255_ioctl(struct file *filep, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd) {
		case SPI_WRITE_CMD:
			...
			writel(status, sdev->membase + SPI_WRITE_REG);
			break;
		case SPI_READ_CMD:
			status = readl(sdev->membase + SPI_READ_REG);
		...
		case SPI_STATUS_CMD:
			...
			put_user(status, (int __user*)arg);
			break;
		case SPI_PRESCALER_CMD:
			get_user(status, (int __user*)arg);
			writel(status, sdev->membase + SPI_PRESCALER_REG);
			break;
		case IQ_SEND_CMD:
			get_user(status, (int __user*)arg);
			writel(status, sdev->membase +IQ_SEND_REG);
			break;
		case ID_CMD:
